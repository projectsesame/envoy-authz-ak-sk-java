...
public class Demo {
    ...
    @GetMapping("/signstr")
    public Mono<ResponseEntity<String>> signstr(@RequestBody(required = false) RequestInfo requestInfo) throws Exception {
        String date = tool.getGMTTime();
        String sign = requestInfo.sign(date);
        return Mono.just(ResponseEntity.status(200).body(sign));
    }
